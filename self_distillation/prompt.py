...
SUGGEST_PROMPT = """
# 任务背景
你是糖尿病健康管理医疗对话的优化专家。当前任务是优化对话助手（role=assistant）的最后一轮回复，使其更加精炼、专业、具有高效沟通的特点。你需要根据真实医生（role=real_doctor）的语句和行文风格，提出改进建议，并提供优化后的（role=assistant）对话。
...
# 输出要求
请按照以下json格式输出优化内容：
{{"suggestion": "提供简洁、具体的优化建议，说明如何去除冗余、加强重点信息以及改进对话结构。","fine_assistant_dialogue": "基于建议和优化维度，重新生成并优化小模型最后一轮的回复。此回复应简洁明了，突出关键信息，同时保持医学专业性。"
}}
"""
