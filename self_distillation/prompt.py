...
SUGGEST_PROMPT = """
# 任务背景
你是一位糖尿病健康管理医疗对话的优化专家，当前任务是优化对话助手（role=assistant）的最后一轮回复，使其更加精炼、专业、具有高效沟通的特点。你需要参照真实医生（role=real_doctor）的语句和行文风格，综合多轮对话上下文，提出改进建议，并提供优化后的（role=assistant）对话回复。

核心目标是：  
**保持医疗准确性 → 消除冗余内容 → 强调关键信息 → 提升对话流畅度和效率**
...
# 优化维度说明：
1. **信息重复度**  
   - **避免无谓重复**：检查是否有多次重复用户已知的信息或医学结论（例如，反复提到“我是AI”）。
   - **去除冗余**：是否有不必要的反复表达同一建议或医学观点？

2. **结构清晰度**  
   - **层次分明**：请确保信息有逻辑层次，核心建议要突出并尽早呈现。
   - **简洁明了**：尽量去除非核心的额外说明，避免冗长的背景解释。

3. **焦点偏移**  
   - **与用户问题相关性**：检查是否包含与当前用户问题无关的内容或过多的额外背景解释。
   - **避免过度泛化**：不要过度解释基础概念或不相关的知识，保持对话紧扣实际问题。

4. **紧急分级与行动导向**  
   - **合理分级**：是否将紧急建议与常规建议进行合理区分，避免混淆？
   - **关键行动清晰**：确保用户能迅速理解需要采取的具体行动，避免细节淹没核心建议。

5. **多轮对话上下文**
   - **考虑对话上下文**：充分考虑多轮对话的上下文信息，确保最后一轮回复与整个对话的一致性和连贯性。

# 输出要求
请按照以下json格式输出优化内容：
{{
"suggestion": "提供简洁、具体的优化建议，说明如何去除冗余、加强重点信息以及改进对话结构。",
"fine_assistant_dialogue": "基于建议和优化维度，重新生成并优化小模型最后一轮的回复。此回复应简洁明了，突出关键信息，同时保持医学专业性，并与多轮对话上下文保持一致。"
}}
"""
...
USER_PROMPT = """
# 任务背景
你是糖尿病健康管理医疗对话的优化专家，当前任务是优化对话，根据下述建议，对下述对话中（role=assistant）所说的话进行优化
# 建议
{suggestion}
# 对话
{dialogue}
...
USER_PROMPT_V2 = """
# 任务背景
你是糖尿病健康管理医疗对话的优化专家，当前任务是优化对话，根据下述问题，参照下述对话并对其进行优化，只输出优化后的对话，不要说其它不相关的话
# 问题
{question}
# 对话
{dialogue}
